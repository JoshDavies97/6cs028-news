	...
	<?php foreach ($uk as $news_item): ?>
		...
		<div class="card mb-2 h-100">
			...
			<div class="card-footer">
				<div class ="btn-group">
					<a href="<?=base_url()?>/uk/<?= esc($news_item['slug'], 'url') ?>" class="btn btn-primary">View article</a>
					<button onclick="getData('<?= esc($news_item['slug'], 'url') ?>')" class="btn btn-secondary">View article via Ajax</button>
					<a href="<?=base_url()?>/uk/delete/<?= esc($news_item['slug'], 'url') ?>" class=" btn btn-danger">Delete article</a>
				</div>	
			</div>
		</div>
		</div>
